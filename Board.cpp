...
void Board::initializeBoard(double frequency, int rows, int columns)
{
  ...
  Board copiedBoard(rows, columns);
...
void Board::initializeBoard(string fileName){
  ...
  Board board(int rows, int columns);
  Board copiedBoard(int rows, int columns);
...
*/
void Board::copyBoard(Board board, Board copiedBoard, int rows, int columns){

  Board maingrid = board;
  Board nextGrid = copiedBoard;
...
printing to the file specified in gapType. It also doesn't check for stagnation or
and empty board because it assumes the user wants everything to be printed to the file, as
